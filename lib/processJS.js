...
Uses Babel and Browserify to load, transpile, and bundle JS files
This could be switched to Rollup in the future if needed.
...
// a Bucket collects text from a Stream
// provides an await-able destination
var Bucket = function() {
  var chunks = [];
...
// allow for importing text
var textExtensions = [".svg", ".html", ".txt", ".glsl"]
var requireString = function(file) {

  if (textExtensions.includes(path.extname(file))) {
    var buffer = [];
    var options = {
      ...
      flush(done) {
        ...
        buffer = [];
        var output = "module.exports = " + JSON.stringify(text);
        this.push(output);
        this.push(null);
        done();
      }
    };
    var transformer = new stream.Transform(options);
    return transformer;
  }
...
module.exports = function(sourceFile, options = {}) {
  return new Promise(async (ok, fail) => {
    var paths = [path.join(path.dirname(sourceFile), "node_modules"), path.join(options.root, "node_modules")];
    var b = browserify({
      debug: true,
      paths
    });
